...
/**
 * Inline SVG icon set (Feather-style, 24px grid). Inlining avoids an icon
 * font or extra requests, which keeps the homepage fast.
 */
...
// Brand (social) icons use filled paths.
const BRAND = {
  ...
  youtube: '<path d="M22.5 6.4a2.8 2.8 0 0 0-2-2C18.8 4 12 4 12 4s-6.8 0-8.5.4a2.8 2.8 0 0 0-2 2A29 29 0 0 0 1 12a29 29 0 0 0 .5 5.6 2.8 2.8 0 0 0 2 2C5.2 20 12 20 12 20s6.8 0 8.5-.4a2.8 2.8 0 0 0 2-2A29 29 0 0 0 23 12a29 29 0 0 0-.5-5.6z"/><path d="M10 15l5-3-5-3z" fill="currentColor" stroke="none"/>',
  x: '<path d="M4 4l16 16"/><path d="M20 4L4 20"/>',
};

function icon(name, size = 24, extraClass = '') {
  const body = PATHS[name] || BRAND[name] || PATHS.check;
  const cls = ['icon', 'icon-' + name, extraClass].filter(Boolean).join(' ');
  return (
    `<svg class="${cls}" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" ` +
    `stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">${body}</svg>`
  );
}
